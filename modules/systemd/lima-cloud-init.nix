...
let
  ...
  script = ''
    ...
    : Mount virtiofs filesystems using systemd-mount for proper automount support
    ${pkgs.yq-go}/bin/yq -r '.mounts[] | select(.[2] == "virtiofs") | "systemd-mount --automount=yes --type=virtiofs --options=" + (.[3] // "rw,nofail,X-mount.mkdir") + " " + .[0] + " " + .[1]' ${LIMA_CIDATA_MNT}/user-data | 
      bash -euxo pipefail
